...
export default function YouTubePlayer() {
    ...
    if(store.currentList){
        if(store.currentList.songs.length > store.playerIndex){
            vidId = store.currentList.songs[store.playerIndex].youTubeId;
        }
    }
    ...
    function loadAndPlayCurrentSong(player) {
        if(store.currentList.songs.length !== 0){
            let song;
            if(store.currentList.songs.length>store.playerIndex){
                song = store.currentList.songs[store.playerIndex].youTubeId;
            } else {
                song = store.currentList.songs[0].youtubeId;
            }
            console.log("ASDASDAS" + song);
